...
# Descriptive statistic of the categoric variables:

# Plotting the category-variable:

ggplot(data=sdata, aes(category_numeric)) + 
  geom_histogram(bindwith = 1000, colour = "blue", fill = "blue") +
  scale_x_continuous(breaks = c(1, 2, 3, 4, 5, 6)) +
  labs(title = "Descriptive statistics for categories",
       x = "Categories",
       y = "Count",
       caption = " N = 401,
      1 = NGOs,
...
ggplot(data=sdata, aes(origin_country_new), bindwith = 50) +
  geom_histogram(colour = "red", fill = "red") + 
  labs(title = "Descriptive statistics for origin country",
       x = "Country of origin",
       y = "Count",
       caption = " N = 401,
      0 = From member state pre-2004,
      1 = From member state post-2004, 
      2 = From non-EU country")
...
# Running an ANOVA-test to check the results, as I have a categorical independent an continous dependent variable:
library(broom, AICcmodavg)

one.way <- aov(success ~ category, data = sdata)
...
h_sal <- sdata[sdata$new_salience == 1, ] # creating a dataframe with only those rows where salience is high

success_by_category_name <- h_sal %>% # to check the success of the different groups on these issues.
  group_by(category) %>% 
  summarise(success_category=mean(success), sucess_sd = sd(success), n = length(category))
...
plot_model(fit, type = "pred", terms = c("fte", "new_salience")) 

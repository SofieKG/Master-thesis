...
install.packages("plm") # installing plm-package.
library(plm)
...
plm_fe_issue <- plm(data = sdata, 
                  success ~ category_factor + fte *as.numeric(new_salience) + brussels_office + 
                    coal_value + origin_country_character + as.numeric(new_salience),
                  na.action = "na.exclude", model = "within", index = "issue", effect = "individual")


summary(plm_fe_issue) # salience is removed as it is dependent on the issue-level.
...
new_sdata <- subset(sdata, success>0 & success<60) # creating a new data frame without the most extreme rows
